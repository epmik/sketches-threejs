...
const VibrantColorsShader = {
	...
	addGuiFolder : function (gui, element, name, openFolder)
	{
		...
		name = name === undefined || name === null ? (isPass ? 'Vibrant Colors Shader Pass' : 'Vibrant Colors Shader Material') : name;

		let folder = gui.addFolder(name);

		let settings = 
		{
			minHue: Math.floor(element.uniforms.minHue.value * -360),
			maxHue: Math.floor(element.uniforms.maxHue.value * 360),
		...
		folder.add(settings, 'maxBrightness', 0, 40, 1).name('Max Brightness').onChange(function (c) { element.uniforms.maxBrightness.value = c *  0.01; });
		folder.add(settings, 'minContrast', 0, 40, 1).name('Min Contrast').onChange(function (c) { element.uniforms.minContrast.value = c * -0.01; });
		folder.add(settings, 'maxContrast', 0, 40, 1).name('Max Contrast').onChange(function (c) { element.uniforms.maxContrast.value = c *  0.01; });
